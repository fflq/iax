...
% read_bf_file: n*( 2size + 1code + data(size) )
% read_bf_packet: 1*( data(len) )
function ret = read_bf_packet(data, read_pci)
    %flq
    len = length(data) ;
	if (nargin < 2)
		read_pci = false ;
...
while cur < (len - 3)
    ...
    if (code == 187) % get beamforming or phy data
        ...
        bytes = data(cur:cur+field_len-2);
        cur = cur + field_len - 1;
    ...
    if (code == 187) %hex2dec('bb')) Beamforming matrix -- output a record
		...
		%fflq
		if (read_pci)
        	ret{count} = read_bfee_with_pci(bytes);
		else
