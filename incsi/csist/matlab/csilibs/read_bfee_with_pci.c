...
void read_bfee(unsigned char *inBytes, mxArray *outCell)
{
	...
	unsigned short pci = inBytes[6] + (inBytes[7] << 8); //fflq
	...
	mxDestroyArray(mxGetField(outCell, 0, "pci")); //fflq
	...
	mxSetField(outCell, 0, "pci", mxCreateDoubleScalar((double)pci)); //fflq
