...
while true
	...
	if (csi_st.pci == 4000)
		...
		fprintf("- %d, %d, 12(%f), 13(%f), %s\n", n, length(g_phaseoff), mean(g_phaseoff(:,1)), mean(g_phaseoff(:,2)), mat2str(csi_st.perm)) ;
		if (r > 0)
...
function r = calc_phaseoff(csi_st)
	...
	fprintf("* calb\n") ;
	csi = Util.cabl_phase(csi, -2.95, -2.85) ;
	perm = csi_st.perm ;
