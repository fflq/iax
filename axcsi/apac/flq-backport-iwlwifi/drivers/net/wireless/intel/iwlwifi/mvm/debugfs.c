...
static ssize_t iwl_dbgfs_disable_rts_write(struct ieee80211_link_sta *link_sta,
					   struct iwl_mvm_sta *mvmsta,
					   struct iwl_mvm *mvm,
					   struct iwl_mvm_link_sta *mvm_link_sta,
					   char *buf, size_t count,
...
static ssize_t iwl_dbgfs_ampdu_size_write(struct ieee80211_link_sta *link_sta,
					  struct iwl_mvm_sta *mvmsta,
					  struct iwl_mvm *mvm,
					  struct iwl_mvm_link_sta *mvm_link_sta,
					  char *buf, size_t count,
					  loff_t *ppos)
{
	...
	iwl_rs_dhc_set_ampdu_size(link_sta->sta, link_sta, ampdu_size);
	return count;
}
...
static ssize_t iwl_dbgfs_flq_monitor_tx_rate_read(struct file *file,
		char __user *user_buf, size_t count, loff_t *ppos)
{
	struct iwl_mvm *mvm = file->private_data;
	char buf[16];
...
static ssize_t iwl_dbgfs_flq_monitor_tx_rate_write(struct iwl_mvm *mvm, 
		char *buf, size_t count, loff_t *ppos)
{
	u32 val;
	int ret;
...
MVM_DEBUGFS_READ_WRITE_FILE_OPS(flq_monitor_tx_rate, 32);
//DEBUGFS_READ_WRITE_FILE_OPS(flq_monitor_tx_rate);
...
#endif
	MVM_DEBUGFS_ADD_FILE(flq_monitor_tx_rate, mvm->debugfs_dir, S_IRUSR | S_IWUSR); //fflq_add
