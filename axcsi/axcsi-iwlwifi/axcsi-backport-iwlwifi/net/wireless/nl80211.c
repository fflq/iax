				...
				const u8 *mac_addr, struct station_info *sinfo)
{
	...
	sinfoattr = nla_nest_start_noflag(msg, NL80211_ATTR_STA_INFO); //fflq
	if (!sinfoattr)
		goto nla_put_failure;
...
static int nl80211_get_reg_do(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_WIPHY]) {
		...
		wiphy = &rdev->wiphy;
		//fflq lar noeffect
		self_managed = wiphy->regulatory_flags & REGULATORY_WIPHY_SELF_MANAGED;
...
//fflqkey, userspace reference, send csi to userspace
				       ...
				       struct netlink_ext_ack *extack)
{
	...
	//fflqkey nl80211_vendor_check_policy debug modify 
	//return 0;
	if (!(attr->nla_type & NLA_F_NESTED)) {
		//fflqkey, here return
...
//fflq nl80211_reg_change_event_fill
