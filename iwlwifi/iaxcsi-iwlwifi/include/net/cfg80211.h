...
#define WIPHY_COMPAT_PAD_SIZE	2560
	...
	//fflqkey, assign in vendor-cmd.c(many vendor_commands)
 ...
 * @gfp: allocation flags
 *
 * fflqkey explain why need netlink notifier register
 ...
 */
static inline struct sk_buff *
cfg80211_vendor_event_alloc_ucast(struct wiphy *wiphy, struct wireless_dev *wdev,
				  unsigned int portid, int approxlen, int event_idx, gfp_t gfp)
{
	//fflq key
	return __cfg80211_alloc_event_skb(wiphy, wdev, NL80211_CMD_VENDOR,
					  NL80211_ATTR_VENDOR_DATA, portid, event_idx, approxlen, gfp);
}
