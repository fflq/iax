...
class subcs_st:
	...
	def gen_subc_common(self):
		self.subcs = np.arange(-self.subcs_radius,self.subcs_radius+subcarrier_st.range_end) 
...
class subcarrier_st:
	...
	def get_vht160_noextra_subc(ntone):
		return subcarrier_st.get_noextra_subc(ntone, 484)
	...
	def get_he160_noextra_subc(ntone):
		#return subcarrier_st.get_noextra_subc(ntone, 2002)
		return subcarrier_st.get_noextra_subc(ntone, 1992)
	...
	@staticmethod
	def set_subc_map():
		subcarrier_st.subc_map = {
			# NOHT
			"NOHT20": subcarrier_st.noht20_subc(),
			# HT
			"HT20": subcarrier_st.ht20_subc(),
			"HT40": subcarrier_st.ht40_subc(),

			# VHT
			"VHT20": subcarrier_st.ht20_subc(),
			"VHT40": subcarrier_st.ht40_subc(),
			"VHT80": subcarrier_st.vht80_subc(),
			"VHT160": subcarrier_st.vht160_subc(),
			# HE
			"HE20": subcarrier_st.he20_subc(),
			"HE40": subcarrier_st.he40_subc(),
			"HE80": subcarrier_st.he80_subc(),
			# HE160 hardly recv, but c-HE160 can
			"HE160": subcarrier_st.he160_subc(),
		}

	@staticmethod
	def get_subc(chan_type_str) -> subcs_st:
		if not subcarrier_st.subc_map:
			subcarrier_st.set_subc_map()
		return subcarrier_st.subc_map[chan_type_str]
	...
	def noht20_subc():
		return subcs_st(
			subcs_radius = 26 ,
			data_pilot_subcs = np.arange(1, 26+subcarrier_st.range_end) ,
	...
	def ht20_subc():
		return subcs_st(
			...
			data_pilot_subcs = np.arange(1, 28+subcarrier_st.range_end) ,
			pilot_subcs = np.array([7, 21]) 
		)
	...
	def ht40_subc():
		return subcs_st(
			subcs_radius = 58 ,
			data_pilot_subcs = np.arange(2, 58+subcarrier_st.range_end) ,
	...
	def vht80_subc():
		return subcs_st(
		    subcs_radius = 122 ,
		    data_pilot_subcs = np.arange(2, 122+subcarrier_st.range_end) ,
	...
	def vht160_subc():
		...
		return subcs_st(
			...
			data_pilot_subcs = np.union1d(
				np.arange(6, 126+subcarrier_st.range_end), 
				np.arange(130, 250+subcarrier_st.range_end)
			),
	...
	def he40_subc():
		...
		return subcs_st(
			subcs_radius = 244,
			data_pilot_subcs = np.arange(3, 244+subcarrier_st.range_end),
	...
	def he80_subc():
		...
		return subcs_st(
			subcs_radius = 500,
			data_pilot_subcs = np.arange(3, 500+subcarrier_st.range_end),
	...
	def he160_subc_doc():
		...
		#null: (8+5+16dc)*2+1
		#so csi(2020) - mid(18) = csi(2002)
		pilot_subcs = subcarrier_st.he80_subc().pilot_subcs + 512 # from 80211ax doc
		return subcs_st(
			subcs_radius = 1012,
			data_pilot_subcs = np.arange(12, 1012+subcarrier_st.range_end),
	...
	def he160_subc():
		...
		return subcs_st(
			subcs_radius = 1012,
			data_pilot_subcs = np.union1d(
				np.arange(12, 509+subcarrier_st.range_end),
				np.arange(515, 1012+subcarrier_st.range_end)
			),
			pilot_subcs = subcarrier_st.he80_subc().pilot_subcs + 512 # from 80211ax doc
		)
