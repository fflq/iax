...
methods (Static)
	...
	function r = get_vht160_noextra_subc(ntone)
		r = subcarrier.get_noextra_subc(ntone, 484);
	end

	%data+pilot should 1992, but csi get 2020
	function r = get_he160_noextra_subc(ntone)
		r = subcarrier.get_noextra_subc(ntone, 1992);
	end
	...
	function subc = get_subc(chan_type_str)
		...
		switch(chan_type_str)
			case "NOHT20"; subc = subcarrier.get_subc_noht20() ;

			case {"HT20", "VHT20"}; subc = subcarrier.get_subc_ht20() ;
			case {"HT40", "VHT40"}; subc = subcarrier.get_subc_ht40() ;

			case "VHT80"; subc = subcarrier.get_subc_vht80() ;
			case "VHT160"; subc = subcarrier.get_subc_vht160() ;

			case "HE20"; subc = subcarrier.get_subc_he20() ;
			case "HE40"; subc = subcarrier.get_subc_he40() ;
			case "HE80"; subc = subcarrier.get_subc_he80() ;
			case "HE160"; subc = subcarrier.get_subc_he160() ;

			otherwise; subc = subcarrier.get_subc_noht20() ;
		end
	end
	...
	function st = gen_subc_common(subcs_radius, pos_data_pilot_subcs, pos_pilot_subcs)
		...
		% preproc
		st.subcs = [-subcs_radius:subcs_radius+subcarrier.range_end] ;
	...
	function st = get_subc_noht20()
		%st = subcarrier.gen_subc_common(26, [1:26+subcarrier.range_end], [7,21], [0]) ;
		st = subcarrier.gen_subc_common(26, [1:26], [7,21]) ;
	end


	% HT
	function st = get_subc_ht20()
		st = subcarrier.gen_subc_common(28, [1:28], [7,21]) ;
	end

	function st = get_subc_ht40()
		st = subcarrier.gen_subc_common(58, [2:58], [11,25,53]) ;
	end


	% VHT
	function st = get_subc_vht80()
		st = subcarrier.gen_subc_common(122, [2:122], [11,39,75,103]) ;
	end

	function st = get_subc_vht160()
		pilot_subcs = [25, 53, 89, 117, 139, 167, 203, 231];
		pilot_subcs = union(pilot_subcs, [190:194]); %self add
		st = subcarrier.gen_subc_common(250, [6:126, 130:250], pilot_subcs);
	end


	% HE
	function st = get_subc_he20()
		st = subcarrier.gen_subc_common(122, [2:122], [22,48,90,116]) ;
	end

	function st = get_subc_he40()
		st = subcarrier.gen_subc_common(244, [3:244], [10,36,78,104,144,170,212,238]) ;
	end

	function st = get_subc_he80()
		st = subcarrier.gen_subc_common(500, [3:500], [24,92,158,226,266,334,400,468]) ;
	end

	function st = get_subc_he160()
		%pilot_subcs = [44,112,178,246,286,354,420,488, 536,604,670,738,778,846,912,980];
		pilot_subcs = subcarrier.get_subc_he80().pilot_subcs + 512; % from 80211ax doc
		pilot_subcs = union(pilot_subcs, [766:770]); %self add
		st = subcarrier.gen_subc_common(1012, [12:509, 515:1012], pilot_subcs);
	end
