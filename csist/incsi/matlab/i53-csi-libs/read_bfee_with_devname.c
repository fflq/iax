...
void read_bfee(unsigned char *inBytes, mxArray *outCell)
{
	...
	//fflq
	char *dev_name = &inBytes[20] ;
	mxArray *mx_dev_name = mxCreateString(dev_name) ;
	//unsigned char *payload = &inBytes[20];
	unsigned char *payload = &inBytes[20+13];
	char tmp;
	...
	//fflq
	mxDestroyArray(mxGetField(outCell, 0, "dev_name"));
	...
	//fflq
	mxSetField(outCell, 0, "dev_name", mx_dev_name) ;
