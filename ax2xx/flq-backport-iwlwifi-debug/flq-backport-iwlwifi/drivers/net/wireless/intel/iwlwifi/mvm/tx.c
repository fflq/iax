			       ...
			       struct ieee80211_sta *sta, __le16 fc)
{
	...
	if (iwl_fw_lookup_cmd_ver(mvm->fw, TX_CMD, 0) > 8) {
		if (!is_cck)
			rate_flags |= RATE_MCS_LEGACY_OFDM_MSK; //fflq, here
		else
...
static u32 iwl_mvm_get_tx_rate_n_flags(struct iwl_mvm *mvm,
				       ...
				       struct ieee80211_sta *sta, __le16 fc)
{
	//fflq 0x8100 = 0x0100 | 0x8000, tx_rate for low 3hex, ants for high 1hex
		...
		if (!sta || !ieee80211_is_data(hdr->frame_control) ||
			...
			rate_n_flags =
				iwl_mvm_get_tx_rate_n_flags(mvm, info, sta,
							    hdr->frame_control);
			/*
			rate_n_flags = 0xc200 ; //fflqkey ht20
			rate_n_flags = 0xca00 ; //fflqkey ht40
			rate_n_flags = 0xc300 ; //fflqkey vht20
			rate_n_flags = 0xcb00 ; //fflqkey vht40
			rate_n_flags = 0xd300 ; //fflqkey vht80
			rate_n_flags = 0xdb00 ; //fflqkey vht160
			//rate_n_flags = 0xc400 ; //fflqkey he20 err
			//rate_n_flags = 0xdc00 ; //fflqkey he160 err
