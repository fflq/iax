...
function test_ppo(st, fid, spec)
	if nargin < 2; fid = 13; end
	if nargin < 3; spec = '-o'; end
	csi1 = squeeze(st.csi(1,1,:));
	csi2 = squeeze(st.csi(1,2,:));
	test_ppo2(csi1, csi2, fid, spec);
end

function test_ppo2(csi1, csi2, fid, spec)
	ppos = unwrap(angle(csi2 .* conj(csi1)));
	figure(fid); hold on; plot(ppos, spec); 
end
