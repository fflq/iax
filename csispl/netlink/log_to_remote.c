...
#define PROCESS_SERVER	"127.0.0.1"
#define PROCESS_PORT	7020
...
typedef void (*handle_cmsg)(struct cn_msg *) ;
enum {
	USE_FILE = 1, 
	USE_UDP
} ;
int handle_mode = USE_FILE ;

int conn_server(const char *server, const int port)
{	
	...
	printf("* conn process[%s:%d]\n", server, port) ;
	while (0 > connect(sfd, (struct sockaddr *)&saddr, sizeof(saddr)))
	{
		perror("connect err\n") ;
		sleep(2) ;
		printf("* conn process[%s:%d]\n", server, port) ;
...
int main(int argc, char** argv)
{
	...
	// flq
	int process_port = PROCESS_PORT ;
	char *process_server = PROCESS_SERVER ;
	if (argc > 1)	process_port = atoi(argv[1]) ;
	if (argc > 2)	process_server = argv[2] ;
	int sfd = conn_server(process_server, process_port) ;
	...
	if (sock_fd == -1)	exit_program_err(-1, "socket");
